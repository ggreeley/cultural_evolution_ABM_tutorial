...
Modify my own custom function (`unbiased_transmission()`) to add generation-to-generation *unbiased* mutation.

```{r}
unbiased_mutation <- function(pop_size,
                              n_generations,
                              initial_prob_trait = NULL,
                              generation_mutate_prob = NULL,
                              n_simulations) {
  ...
  for (i in 1:n_simulations) {
    ...
    output$initial_prob_trait <- initial_prob_trait
      ...
      for (t in 2:n_generations) {
        ...
        # mutate probability, drawn from runit(0,1)
        mutate_probs <- runif(pop_size)
        
        # if agent was A, with probability mu, flip to B
        agent$trait[previous_agent$trait == "A" & mutate_probs < generation_mutate_prob] <- "B"  
      
        # if agent was B, with probability mu, flip to A
        agent$trait[previous_agent$trait == "B" & mutate_probs < generation_mutate_prob] <- "A" 
...
```{r}
um_test <- unbiased_mutation(pop_size = 100,
                  n_generations = 100,
                  initial_prob_trait = 0.99,
                  generation_mutate_prob = 0.05,
                  n_simulations = 20)

bind_rows(um_test) |>
  ggplot(aes(x = n_itr, y = p, group = n_sim)) +
  geom_line()
```

Extend to allow for various generation mutation probabilities

```{r}

unbiased_mutation_mult_prob <- function(initial_prob_grid = NULL) {
  
  store_list <- list()
  
  for (i in initial_prob_grid) {
  
  sim_dat <- unbiased_mutation(pop_size = 100,
                               n_generations = 100,
...
```{r}
um_mult_test <- unbiased_mutation_mult_prob(initial_prob_grid = seq(from = 0.1,
                                                                    to = 0.9,
                                                                    by = 0.1))

prob_a_mutate_range_mod <- um_mult_test |>
  bind_rows() |>
...
prob_a_mutate_range_mod |>
  ...
  labs(title = "Population of 100 Across 100 Geneartions\n20 Simulated Evo Cycles w/ Mutate Prob = 0.05",
       x = "Generation",
       y = "% of Population w/ Trait 'A'",
       color = "Sample Preference for Trait 'A'") +
  theme_light()
```
...
```{r}
biased_mutation <- function(pop_size,
                              n_generations,
                              initial_prob_trait = NULL,
                              generation_mutate_prob = NULL, # 'a' preference
                              n_simulations) {
  ...
  for (i in 1:n_simulations) {
    ...
    output$n_itr[1] <- 1
    output$initial_prob_trait <- initial_prob_trait
    output$generation_mutate_prob <- generation_mutate_prob
...
```{r}
biased_mutation_mult_prob <- function(initial_prob_grid = NULL) {
  
  store_list <- list()
  
  for (i in initial_prob_grid) {
  
  sim_dat <- biased_mutation(pop_size = 100,
                               n_generations = 100,
                               initial_prob_trait = i,
                               generation_mutate_prob = 0.05,
                               n_simulations = 20)
...
# possible to make version of this function that also cycles through 
# mutation probability values, e.g., argument for mutate_prob_grid = NULL)
biased_mutation_mult_flex <- function(initial_prob_grid = NULL,
                                 mutate_prob_grid  = NULL) {
  
  store_list <- list()
  
  for (i in initial_prob_grid) {
    
    for (j in mutate_prob_grid) {
      
      sim_dat <- biased_mutation(pop_size = 100,
                               n_generations = 100,
                               initial_prob_trait = i,
                               generation_mutate_prob = j,
                               n_simulations = 20)
...
#MULT_trial <- biased_mutation_MULT(initial_prob_grid = c(0.25, 0.75),
#                                   mutate_prob_grid = c(0.05, 0.20))
```

Test `biased_mutation_mult_prob()`

```{r}
bm_mult_test <- biased_mutation_mult_prob(initial_prob_grid = seq(from = 0.1, to = 0.9, by = 0.1))

prob_a_bimutate_range_mod <- bm_mult_test |>
  bind_rows() |>
  mutate(full_info = str_c(n_sim, n_itr, initial_prob_trait, sep = "_"),
         sim_prob_info = str_c(n_sim, initial_prob_trait, sep = "_"),
         sim_prob_info = as.factor(sim_prob_info),
...
Test `biased_mutation_mult_flex()`, which allows for specifying a range of both a) starting generation trait prevalence and b) mutation probabilities. If this feature is always desired, could place arguments from `biased_mutation()` into this function for controling other parameters as well.

```{r}
biased_mutation_mult_flex()
```
