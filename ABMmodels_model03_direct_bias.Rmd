...
```{r}
biased_transmission <- function(pop_size,
                                n_generations,
                                initial_prob_trait  = NULL,
                                # if rand prev. gen = A, copy with this prob:
                                # hence, biased *transmission* from prev. gen
                                s_transmission_prob = NULL,
                                n_simulations) {
  ...
  for (i in 1:n_simulations) {
    ...
    output$s_transmission_prob <- s_transmission_prob
      ...
      for (t in 2:n_generations) {
        ...
        # for each, randomly sample agent from previous generation
        demonstrator <- sample(previous_agent$trait, size = pop_size, replace = FALSE)
        
        # uniform distribution of transmission probs
        transmission_probs <- runif(pop_size)
      
        # if agent was A, with adopt with probability s
        agent$trait[demonstrator == "A" & transmission_probs < s_transmission_prob] <- "A" 
...
```{r}
test <- biased_transmission(pop_size = 100,
                    n_generations = 100,
                    initial_prob_trait = 0.10,
                    s_transmission_prob = 0.5,
                    n_simulations = 10)
