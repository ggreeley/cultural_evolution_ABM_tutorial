...
Note the title of the graph, whic
h gives the value of $N$ for this simulation. It's easy to create plots and forget what the parameters were. It's therefore a good idea to include it somewhere on the graph. There's no need to include our other parameter, $t_{max}$, because that can be seen from the x-axis scale.
...
UnbiasedTransmission <- function (N, t_max) {
  ...
  # i'll do my own plotting with the created data object
  #plot(output$p, 
  #     type = 'l', 
  #     ylab = "p, proportion of agents with trait A", 
  #     xlab = "generation", 
...
**ADDED BY GG**: Extended function (created before I saw similar extended function below) to allow for specifying number of simulations. 

```{r}

unbiased_transmission <- function(pop_size,
                                  n_generations,
                                  initial_prob_trait = NULL,
                                  n_simulations) {
  ...
  for (i in 1:n_simulations) {
    ...
    agent <- data.frame(trait = sample(c("A","B"), pop_size,
                                       prob = c(initial_prob_trait, 1-initial_prob_trait),
                                       replace = TRUE))
  
    output <- data.frame(p = rep(NA, n_generations),
                         n_sim = rep(NA, n_simulations))
  
    output$p[1] <- sum(agent$trait == "A") / pop_size
    output$n_sim <- i
    output$n_itr[1] <- 1
    output$initial_prob_trait <- initial_prob_trait
      ...
      for (t in 2:n_generations) {
        ...
        # randomly copy from previous generation
        agent <- data.frame(trait = sample(previous_agent$trait, pop_size, replace = TRUE))  
  
        # get p and put it into output slot for this generation t
        output$p[t] <- sum(agent$trait == "A") / pop_size 
        
        # track the iteration
        output$n_itr[t] <- t 
        
        # place output data in list
        store[[i]] <- output
    }
  }
  return(store)
...
#unbiased_transmission(pop_size = 100, n_generations = 100, prob_trait = 0.5, n_simulations = 5)
```

Generate some data

```{r}
# 100 independent agents in each of the 100 generations
# trait "A" (vs "B") is selected from the previous with prob = 0.25
  # repeat evolution process 50 times
prob_a_25 <- unbiased_transmission(pop_size              = 100,
                                   n_generations         = 500,
                                   initial_prob_trait    = 0.25,
                                   n_simulations         = 50)

prob_a_50 <- unbiased_transmission(pop_size              = 100,
                                   n_generations         = 500,
                                   initial_prob_trait    = 0.50,
                                   n_simulations         = 50)

prob_a_75 <- unbiased_transmission(pop_size              = 100,
                                   n_generations         = 500,
                                   initial_prob_trait    = 0.75,
                                   n_simulations         = 50)

# add some flexibility by allowing different prob_trait values
unbiased_transmission_mult_prob <- function(initial_prob_grid = NULL) {
  
  store_list <- list()
  
  for (i in initial_prob_grid) {
  
  sim_dat <- unbiased_transmission(pop_size          = 100,
                               n_generations         = 500,
                               initial_prob_trait    = i,
                               n_simulations         = 50)
  
  store_list[[length(store_list) + 1]] <- sim_dat
  
  }
  return(store_list)
}

# for each probability (prop) in prob_grid,
# simulate 50 each:
  # population of 100 across 500 generations, 
  # each individual in each generation samples trait A with prob i from previous generation
prob_a_range <- unbiased_transmission_mult_prob(initial_prob_grid = seq(from = 0.1,
                                                                        to   = 0.9,
                                                                        by   = 0.1))
```
...
list(prob_a_25,
     ...
     prob_a_75) |>
  bind_rows() |>
  mutate(full_info = str_c(n_sim, n_itr, initial_prob_trait, sep = "_"),
         sim_prob_info = str_c(n_sim, initial_prob_trait, sep = "_"),
         sim_prob_info = as.factor(sim_prob_info),
         n_sim = as.factor(n_sim),
         initial_prob_trait = as.factor(initial_prob_trait)) |>
  ggplot(aes(x = n_itr, y = p, color = initial_prob_trait)) +
  geom_line(aes(group = sim_prob_info)) +
  facet_wrap(~ initial_prob_trait) +
  scale_x_continuous(breaks = c(1, 100, 200, 300, 400, 500)) +
  labs(title = "Population of 100 Across 500 Geneartions",
       x = "Generation",
       y = "% of Population w/ Trait 'A'",
       color = "Trait 'A' Prevalence at Gen = 1") +
  theme_light()

# confirm right number of rows for each prob_trait
prob_a_range |>
  bind_rows() |>
  group_by(initial_prob_trait) |>
  count()

# add and/or aggregate data for further exploration
prob_a_range_mod <- prob_a_range |>
  bind_rows() |>
  mutate(full_info = str_c(n_sim, n_itr, initial_prob_trait, sep = "_"),
         sim_prob_info = str_c(n_sim, initial_prob_trait, sep = "_"),
         sim_prob_info = as.factor(sim_prob_info),
         n_sim = as.factor(n_sim),
         initial_prob_trait = as.factor(initial_prob_trait)) |> 
  group_by(initial_prob_trait, n_itr) |>
  mutate(p_itr_mean = mean(p),
         p_itr_plus_se = p_itr_mean + (sd(p) / sqrt(50)),
         p_itr_minus_se = p_itr_mean - (sd(p) / sqrt(50))) |>
  ungroup() |> 
  group_by(n_sim, initial_prob_trait) |>
  mutate(gen_complete_1 = which(p == 1)[1],
         gen_complete_0 = which(p == 0.00)[1]) |>
  ungroup()

prob_a_range_mod |>
  ggplot(aes(x = n_itr, y = p)) +
  geom_line(aes(group = sim_prob_info),
            color = "grey") +
  geom_line(aes(x = n_itr, y = p_itr_mean, group = initial_prob_trait),
            color = "darkred") +
  geom_ribbon(aes(ymin = p_itr_minus_se, ymax = p_itr_plus_se),
              fill = "darkred",
              color = "white",
              alpha = 0.2) +
  facet_wrap(~ initial_prob_trait) +
  scale_x_continuous(breaks = c(1, 100, 200, 300, 400, 500)) +
  labs(title = "Population of 100 Across 500 Geneartions\n50 Simulated Evolutions",
       x = "Generation",
       y = "% of Population w/ Trait 'A'",
       color = "Trait 'A' Prevalance at Gen = 1") +
  theme_light()

prob_a_range_mod |>
  select(initial_prob_trait, n_sim, gen_complete_0, gen_complete_1) |>
  group_by(initial_prob_trait, n_sim) |>
  slice_head(n = 1) |>
  ungroup() |>
  pivot_longer(cols = starts_with("gen"),
               names_to = "gen_complete",
               values_to = "gen_to_complete") |>
  group_by(initial_prob_trait, gen_complete) |>
  mutate(mean_gen_finish = mean(gen_to_complete, na.rm = TRUE)) |>
  ungroup() |> 
  ggplot(aes(x = gen_to_complete, fill = gen_complete)) +
  geom_histogram() +
  geom_vline(aes(xintercept = mean_gen_finish, color = gen_complete)) +
  facet_wrap(~ initial_prob_trait) +
  theme_light()
```
